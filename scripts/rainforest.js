(function initRainforest() {
  ...
  const skyPresets = {
    ...
    rainforestMist: {
      horizon: ["#FFFDF2", "#F8F8E8", "#EEF6DA"],
      mid: ["#7FE0B8", "#4ECDA0", "#3AB88A"],
      zenith: ["#1E8A7A", "#167A72"],
      layers: [
        { color: "#FFF9E0", opacity: 0.34, speed: 0.14, scale: 1.35, y: 0.66 },
        { color: "#9FE8D8", opacity: 0.26, speed: 0.08, scale: 2.05, y: 0.42 },
        { color: "#6FD4C8", opacity: 0.18, speed: 0.04, scale: 2.65, y: 0.26 }
      ]
    }
  };
  ...
  window.__rainforestSwaySpeed = 1;
  window.__rainforestCustomPlants = window.__rainforestCustomPlants || [];

  function wireRainforestControls() {
    const swayEl = document.getElementById("sway-speed");
    const swayReadout = document.getElementById("sway-speed-readout");
    if (swayEl) {
      const applySway = () => {
        const v = parseFloat(swayEl.value);
        window.__rainforestSwaySpeed = Number.isFinite(v) ? v : 1;
        if (swayReadout) {
          swayReadout.textContent =
            window.__rainforestSwaySpeed < 0.85
              ? "slow"
              : window.__rainforestSwaySpeed > 1.35
                ? "fast"
                : "medium";
        }
      };
      swayEl.addEventListener("input", applySway);
      applySway();
    }

    document.getElementById("clear-planted")?.addEventListener("click", () => {
      window.__rainforestCustomPlants = [];
    });
  }

  wireRainforestControls();
})();
