...
function initRainforestPlants(containerId) {
  ...
  new p5(function(p) {
    ...
    function syncPlantLayerSize() {
      ...
      H = Math.max(1, Math.floor(r.height)) || W;
      if (p.canvas) {
        p.resizeCanvas(W, H);
      }
    }
    ...
    const PLANT_SCALE = 0.52;
    const VINE_COUNT = 16;

    function getSwayMult() {
      const v = Number(window.__rainforestSwaySpeed);
      if (!Number.isFinite(v)) return 1;
      return Math.max(0.12, Math.min(2.6, v));
    }
    ...
    function makeAnimatedPt(rawPts, vineIdx) {
      ...
      return function animatedPt(tPos) {
        const t = p.frameCount * 0.018 * getSwayMult();
    ...
    function drawHeartLeaf(cx, cy, size, angle, tint, pulse = 0) {
      ...
      if ((tint + Math.floor(cx * 0.02)) % 8 === 0) {
        p.noStroke();
        p.fill(248, 252, 220, 72);
        p.ellipse(-s * 0.16, -s * 0.2, s * 0.45, s * 0.28);
      }
    ...
    function drawPlantedMiniVine(cx, cy, seed) {
      p.randomSeed((seed >>> 0) % 999_983);
      const sm = getSwayMult();
      const tAnim = p.frameCount * 0.018 * sm;
      const sway = Math.sin(tAnim * 1.05 + (seed % 1200) * 0.01) * 0.11;
      const stemH = Math.min(H, W) * 0.1;
      p.push();
      p.translate(cx, cy);
      p.rotate(sway);
      p.stroke(STEM_C[0] - 8, STEM_C[1] - 10, STEM_C[2] - 6, 195);
      p.strokeWeight(Math.max(W * 0.0026, 0.9));
      p.line(0, 0, 0, -stemH);
      const offs = [-0.038, 0, 0.038];
      const sizes = [0.015, 0.017, 0.015];
      for (let i = 0; i < 3; i += 1) {
        const pulse = Math.sin(tAnim * 1.25 + i + seed * 0.001) * 0.1;
        drawHeartLeaf(
          offs[i] * W,
          -stemH * 0.72,
          sizes[i],
          -p.HALF_PI + (i - 1) * 0.28 + pulse,
          (seed + i) % 3,
          pulse
        );
      }
    ...
    function drawCustomPlants() {
      const arr = window.__rainforestCustomPlants;
      if (!arr || !arr.length) return;
      arr.forEach((pl) => {
        drawPlantedMiniVine(pl.nx * W, pl.ny * H, pl.seed);
      });
    }
    ...
    p.setup = function() {
      ...
      p.mousePressed = function mousePressed() {
        ...
        if (!window.__rainforestCustomPlants) window.__rainforestCustomPlants = [];
        if (window.__rainforestCustomPlants.length >= 12) return;
        const seed = ((p.mouseX * 73819) ^ (p.mouseY * 50841)) >>> 0;
        window.__rainforestCustomPlants.push({
          nx: p.mouseX / p.width,
          ny: p.mouseY / p.height,
          seed
        });
      };
    };
